...
from spotymatebot.database.db import get_user_language, save_user_language
from spotymatebot.utils.i18n import get_message
from spotymatebot.services.spotify import process_spotify_link
import re
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_language:
        await update.message.reply_text(
            get_message(current_language, "welcome").format(language=current_language.upper())
        )
    else:
        keyboard = [
            [
